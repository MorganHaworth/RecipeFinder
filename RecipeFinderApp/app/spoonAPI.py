...
'''getAllResultsFormatted()=============================================================
    Desc: returns a string of each recipe's title, img src, and ingredients it uses
    Param:
        resultArr: A list/array of recipes. Each array index is a recipe dict.
    Return:
...
def getInstructionsById(id):
    ...
    try:
        ...
        instructions = f"{resultsDict['summary']}\n {resultsDict['instructions']}"
    except:
...
def getTitleById(resultsArr, id):
    ...
    for x in resultsArr:
        if x['id'] == id:
            recipeTitle += (f"={x['title']}") #Each x is a dict. Get the varibles that we need
            break
    return recipeTitle
